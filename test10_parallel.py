...
def run_single_testcase(test_data):
    """
    獨立的測試執行函式 (Worker Function)。
    這段函式會在不同的 CPU 核心的獨立 Process 中執行。
    """
    name, arr = test_data

    # 每個 Process 必須建立自己的 Engine 實體，避免跨行程的物件鎖定問題
...
if __name__ == "__main__":
    ...
    print(f"=== 啟動多進程並行測試 (CPU 核心數: {max_workers}) ===")

    total_start_time = time.time()
    results_map = {}

    # 啟用 ProcessPoolExecutor 進行多核心並行分發
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 將 test_cases 映射到 worker 函式中
        # executor.map 會保留原本陣列的順序，這對於印出報表非常方便
        future_results = executor.map(run_single_testcase, test_cases)

        # 收集結果
        for name, res in future_results:
